...
<?= '<h4>'. __( 'Moje objednávky' , 'rootscope' ) .'</h4>' ?>
    ...
    <tbody>
        ...
        <tr
            ...
            <td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>"
                ...
                <span class="order-legend"><?= __('Číslo objednávky','rootscope') ?></span>
                <?php echo esc_html( _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() ); ?>
                <?php elseif ( 'order-date' === $column_id ) : ?>
                <span class="order-legend"><?= __('Dátum objednania','rootscope') ?></span>
                <time
                ...
                <span class="order-legend"><?= __('Stav objednávky','rootscope') ?></span>
                <?php echo esc_html( wc_get_order_status_name( $order->get_status() ) ); ?>
                <?php elseif ( 'order-total' === $column_id ) : ?>
                <span class="order-legend"><?= __('Cena','rootscope') ?></span>
                <?php
                ...
                <?php
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button button color ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
								}
								?>
                <?php endif; ?>
